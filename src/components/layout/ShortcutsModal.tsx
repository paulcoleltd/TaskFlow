...
function Kbd({ children }: { children: string }) {
  return (
    <kbd className="inline-flex items-center justify-center min-w-[28px] h-7 px-1.5 rounded-lg bg-[#0B1437] border border-[#1F3461] text-xs font-mono text-slate-300 shadow-inner">
      {children}
    </kbd>
  );
}
            ...
            <div key={group} className="mb-4 last:mb-0">
              ...
              <div className="space-y-0">
                {items.map(({ keys, description }) => (
                  <div key={description} className="flex items-center justify-between gap-4 py-2.5 border-b border-[#1F3461] last:border-0">
