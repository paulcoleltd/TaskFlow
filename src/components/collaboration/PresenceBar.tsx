    ...
    <div className="hidden sm:flex items-center gap-1" aria-label="Online collaborators">
      {shown.map((user) => (
        <Tooltip key={user.userId} content={`${user.userName} is online`}>
          <div className="relative flex-shrink-0">
            <div
              className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold text-white ring-2 ring-emerald-400/70 ring-offset-1 ring-offset-[#06091A]"
              style={{ backgroundColor: user.userColour }}
            >
              {getInitials(user.userName)}
            </div>
            {/* Green presence dot */}
            <span className="absolute bottom-0 right-0 w-2 h-2 bg-emerald-400 border border-[#06091A] rounded-full" />
          </div>
        </Tooltip>
      ))}
      ...
      {overflow > 0 && (
        <Tooltip content={`${overflow} more ${overflow === 1 ? 'person' : 'people'} online`}>
          <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold text-slate-400 bg-[#0C1526] border border-[#1C3054]">
            +{overflow}
          </div>
        </Tooltip>
      )}
